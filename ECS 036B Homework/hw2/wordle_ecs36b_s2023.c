...
// for ecs36b s2023 students --
// the following functions are important to know, in order to get an overall picture
...
help()
{
  printf("Wordle is a simple game:" EOL "Guess the %i letter word within infinite tries" EOL,
	 WORD_LENGTH);
  puts(
...
gameLoop()
{
 ...
 // int scan = 0;
  ...
  // ecs36b ==> important loop to understand!!
...
#define Felix_HACK
    ...
    fscanf(fpSol, "%s", buffer);
	  ...
	  if (strcmp(buffer, word) == 0) 
	    {
	      printf("we already won. The word was %s\n", word);
	      exit(0);
	    }
      ...
      if ((scan = scanf("%5s", guess)) == 1 && strlen(guess) == WORD_LENGTH)
	{
	  ...
	  if (strcmp(guess, word))
	      ...
	      if (isWord(guess) && hasWord(guess))
		{
		  ...
		  // ecs36b ==> checkWord() ==> try to understand
		  if (checkWord(guess))
		    {
		      removeAlpha(guess);
		      printf("\t%s\n", alpha);
...
int
strpos(const char * str, char search)
{
  int i = 0;
  if (str != NULL)
    {
...
// for ecs36b students, you probably don't need to worry about setup
long
setup()
{
  FILE* cache;
