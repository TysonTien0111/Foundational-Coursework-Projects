...
for part in "google.com".encode("ascii").split(b"."):
...
with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as dns_client:
    ...
    print(f"Parsed DNS Records, TLD DNS server found. TLD DNS server is at ('192.41.162.30', 53) and DNS type is A.\n")

    start_time = datetime.now()
    dns_client.sendto(dns_query, ("192.41.162.30", 53))
    print("Sending out DNS Query to TLD DNS server ('192.41.162.30', 53).")
    ...
    print(f"Parsed DNS Records, Authoritative DNS server found. Authoritative DNS server is at ('216.239.34.10', 53) and DNS type is A.\n")

    start_time = datetime.now()
    dns_client.sendto(dns_query, ("216.239.34.10", 53))
    print("Sending out DNS Query to Authoritative DNS server ('216.239.34.10', 53).")
    ...
    print(f"Parsed DNS Records, \"google.com\" server found. \"google.com\" server is at ('142.251.214.142', 53) and DNS type is A.\n")

    print(f"Total RTT: {total_time} in seconds.\n")

with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as http_client:
    http_client.connect(("142.251.214.142", 80))
    start_time = datetime.now()
    http_client.sendall(b"GET / HTTP/1.1\r\nHost:142.251.214.142\r\n\r\n")
